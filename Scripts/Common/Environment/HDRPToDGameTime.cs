...
namespace DaftAppleGames.Common.Environment
{
    ...
    public class HDRPToDGameTime : BaseGameTime
    {
        ...
        {
            _currentHour = (int)HDRPTimeOfDay.Instance.TimeOfDay;
            return _currentHour;
        } // GetHour()
        ...
        override public void SetHour(float hour)
        {
            ...
            HDRPTimeOfDay.Instance.TimeOfDay = ((int)hour);
        } // SetHour()

        override public void SetMinute(float minute)
        {
            float currentTime = HDRPTimeOfDay.Instance.TimeOfDay;
            float currHour = (int)currentTime;
            HDRPTimeOfDay.Instance.TimeOfDay = currHour + (60 / minute);
